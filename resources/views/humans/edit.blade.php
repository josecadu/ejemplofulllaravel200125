...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg">
    <h2 class="text-2xl font-semibold text-center mb-6">Formulario Modificación Usuario</h2>

    <form action="{{route('humans.update', $human)}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('put')
        <!-- Campo de nombre de usuario -->
        <div class="mb-4">
            <label for="username" class="block text-sm font-medium text-gray-700">Nombre de Usuario</label>
            <input type="text" id="username" value="{{@old('username', $human->username)}}" 
        ...
        <div class="mb-4">
            ...
            <input type="email" id="email" name="email" value="{{@old('email', $human->email)}}"
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center space-x-6">
                <label class="flex items-center space-x-2">
                    <input type="radio" name="activo" value="SI" @checked(old('activo', $human->activo)=='SI') 
                    class="form-radio h-4 w-4 text-indigo-600">
                    <span>SI</span>
                </label>
                <label class="flex items-center space-x-2">
                    <input type="radio" name="activo" value="NO" @checked(old('activo', $human->activo)=='NO') 
                    class="form-radio h-4 w-4 text-indigo-600">
                    <span>NO</span>
                </label>
            </div>
        ...
        <div class="mb-4">
            ...
            <select id="departament_id" name="departament_id" class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                ...
                @foreach ($departamentos as $departamento)
                    <option value="{{$departamento->id}}" @selected(old('departament_id', $human->departament_id)==$departamento->id)>{{$departamento->nombre}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center space-x-6">
                ...
                <label class="flex items-center space-x-2 italic" for={{$rol->id}}>
                    <input id="{{$rol->id}}" type="checkbox" name="role_id[]" @checked(in_array($rol->id, old('role_id', $rolesUsuario))) 
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center">
                ...
                <img id="preview" src="{{Storage::url($human->logo)}}" alt="Vista previa" class="ml-4 w-16 h-16 object-cover">
            </div>
        </div>
        <x-error for="logo" />
